...
int search(struct node *save1, struct node *save2){
    ...
    int left = search(save1->lptr, save2->lptr);
    if(left == 0) return 0;
    int right = search(save1->rptr, save2->rptr);
    if(right == 0) return 0;
}
...
void main(){
    int n;
    //insert
    printf("Enter root node of tree 1 : ");
    scanf("%d",&n);
    
    //create root node
    struct node *root1 = createNode(n);

    //creae BST
    root1 = createTree(root1);

    //display
    printf("-------Inorder of tree 1 : ");
    inOrder(root1);

    //---------------------------------------
    
    //insert
    printf("\n\nEnter root node of tree 2 : ");
    scanf("%d",&n);
    
    //create root node
    struct node *root2 = createNode(n);

    //creae BST
    root2 = createTree(root2);

    printf("-------Inorder of tree 2: ");
    inOrder(root2);
    
    //check
    if(search(root1,root2) == 1) printf("\nBoth the tree are same");
    else printf("\nBoth the tree are not same");
}